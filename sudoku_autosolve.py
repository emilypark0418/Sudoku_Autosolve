...
class SudokuSolver():
    ...
    def valid(self,x,y,num):
        ...
        b = 3 *((y-1)//3)
        for z in range(1,10):
              list.append(self.puzzle[x-1][z-1])
              list.append(self.puzzle[z-1][y-1])
                
        for p in range(a, a+3):
            for q in range(b, b+3):
                list.append(self.puzzle[p][q])
    ...
    def solver(self):
        ...
        for num in range(1,10):
            if(self.valid(x,y,num)):
                self.problem.display(x,y,num)
                self.puzzle[x-1][y-1]=num
                if(self.solver()):
                    self.problem.is_done(self.puzzle)
                    return True
                    
                else:   
                    self.puzzle[x-1][y-1]=0
                    self.problem.display(x,y,"")    
        return False
